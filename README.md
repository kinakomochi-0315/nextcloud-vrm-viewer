...
公開共有リンク、VRMA、モーション再生、表情編集、サーバーサイドサムネイルは0.1.0の対象外です。

## インストール

GitHub Releaseの`files_vrmviewer-0.1.0.tar.gz`をNextcloudの`custom_apps`へ展開します。

```bash
tar -xzf files_vrmviewer-0.1.0.tar.gz -C /path/to/nextcloud/custom_apps
...
`build/artifacts/files_vrmviewer-0.1.0.tar.gz`とSHA-256ファイルが生成されます。
